...
class StylePreferenceSummary(BaseModel):
    """Structured style preference extracted from user text and optional reference image."""

    search_query: str = Field(
        ...,
        description="A concise CLIP-friendly fashion image retrieval query.",
    )
    category: Optional[str] = Field(
        default=None,
        description="Optional garment category such as top, dress, pants, jacket, shoes, bag.",
    )
    style_note: str = Field(
        ...,
        description="Detailed summary of the user's desired style.",
    )

class SummarizePreferenceInput(BaseModel):
    """Input for summarizing user fashion preference."""

    user_message: str = Field(..., description="User's fashion request.")
    style_image_url: Optional[str] = Field(
        default=None,
        description="Optional reference style image URL.",
    )
...
class ToolFactory:
    ...
    def build_tools(self):
        ...
        preference_model = ChatOpenAI(
            model="gpt-4.1-mini",
            temperature=0.2,
        )

        @tool(args_schema=SummarizePreferenceInput)
        def summarize_preference(user_message: str, style_image_url: Optional[str] = None) -> str:
            """Summarize the user's text request and optional reference style image into a fashion retrieval preference.

            Use this when the user gives a new clothing/style request, provides a reference image,
            or changes their style preference before try-on.

            Do not use this when the user is merely selecting a previous result,
            such as "try the second one", "I choose option 3", or "use the first item".
            """
            structured_model = preference_model.with_structured_output(StylePreferenceSummary)

            instruction = """
            You are a fashion preference extraction assistant.

            Given the user's text request and optional reference style image, summarize the user's clothing preference for image retrieval. 
            Extract user's preference from both text request and reference style image, dont ignore the reference style image information.
            ...
            Return a concise CLIP-friendly fashion retrieval query that is composed of several keywords.
            """.strip()
            ...
            pref = structured_model.invoke(messages)
            return pref.model_dump_json(indent=2)
        ...
        return [summarize_preference, search_fashion_images, virtual_tryon]
...
1. summarize_preference
Use this when the user gives a new fashion request, provides a reference style image,
or changes their style preference before try-on.
This tool returns:
- search_query
- category
...
2. search_fashion_images
Use this to search for candidate fashion product/reference images.
Usually, first call summarize_preference, then use its search_query and category
to call search_fashion_images.

3. virtual_tryon
Use this only when the user has both:
- a person image URL
- a garment image URL

Important behavior rules:
- If the user gives a new clothing/style request, call summarize_preference first, then search_fashion_images.
- After calling summarize_preference, explicitly show the user the extracted preference before presenting results.
- Show it in this format:

Required final response format after recommendation search:
Extracted preference:
- Search query: ...
- Category: ...
- Style note: ...

- Then call search_fashion_images using the Search query and Category above.
- If the user provides a reference style image URL, pass that URL into summarize_preference.
- If the user changes preference, call summarize_preference again and then search_fashion_images again.

Try-on rules:
- If the user provides both a person image URL and a garment image URL, call virtual_tryon immediately.
- Do NOT ask for garment_type, style_note, or size_hint before calling virtual_tryon.
- garment_type, style_note, and size_hint are optional.
- If garment_type is obvious from the user message, infer it.
- If garment_type is not obvious, pass garment_type=None.
- If style_note is not given, pass style_note=None.
- If size_hint is not given, pass size_hint=None.
- If the user wants try-on but the person image URL is missing, ask for the person image URL.
- If the user wants try-on but the garment image URL is missing, ask for the garment image URL.

Other rules:
- Do NOT call summarize_preference when the user is merely selecting a previous result,
  e.g. "try the second one", "I choose option 3", or "use the first item".
- Never invent final try-on images.
...
class VirtualTryOnAgent:
    ...
    def invoke(self, request: AgentRequest) -> Dict[str, Any]:
        ...
        if request.style_image_url:
            content += f"\n\nReference style image URL: {request.style_image_url}"
...
def main() -> None:
    ...
    demo_turns = [
        {
            "title": "Demo 1: text request + reference style image",
            "user_message": (
                "Find me some black dresses. "
                # "Find me some minimalist black dresses for office wear. "
                # "I prefer a clean, elegant, not too revealing style."
            ),
            "style_image_url": "http://assets.myntassets.com/v1/images/style/properties/59df6e5a848ef0d7cfa409e55ad7cf60_images.jpg",
        },
        # {
        #     "title": "Demo 2: user changes preference before try-on",
        #     "user_message": (
        #         "Actually, make it more casual and suitable for a summer date."
        #     ),
        #     "style_image_url": None,
        # },
        {
            "title": "Demo 3: user provides explicit person image and garment image for try-on",
            "user_message": (
                "I want to try this item on. "
                "My person image URL is http://assets.myntassets.com/v1/images/style/properties/59df6e5a848ef0d7cfa409e55ad7cf60_images.jpg. "
                "The garment image URL is http://assets.myntassets.com/v1/images/style/properties/bfb5912d10eccf37087a9689556e61e2_images.jpg."
            ),
            "style_image_url": None,
        },
    ]
    ...
    for i, turn in enumerate(demo_turns, start=1):
        ...
        if turn["style_image_url"]:
            print(f"REFERENCE STYLE IMAGE: {turn['style_image_url']}")

        print()

        result = app.run(
            user_message=turn["user_message"],
            style_image_url=turn["style_image_url"],
        )
